...
void AddBookWithoutAuthor()
{
    ...
    _context.SaveChanges(); // This Line will Throw Exception As AuthorId Property Must not be null as Integer value
}

void GetAuthors()
{
    // Search for Author and filter by Variable to See the Query Log's output
    // Does display Parameter's value or not
    var name = "Ozeki";
    var authors = _context.Authors.Where(author => author.LastName == name).ToList();

    authors.ForEach(author => Console.WriteLine($"{author.FirstName} {author.LastName}"));
}

Console.WriteLine("GetAuthors Method Execute =======");
GetAuthors();
